...
#2.Arranging with arrange() and desc()
```{r}
bikes_tbl %>%
  select(model, price) %>%
  arrange(desc(price)) %>%
...
#5.Grouping & Summarizing with group_by() and summarize()
...
#6.Renaming columns with rename() and set_names()
...
#7.Reshaping (Pivoting) Data with spread() and gather()
...
#8.Joining Data by Key(s) with left_join() (e.g. VLOOKUP in Excel)
```{r}
orderlines_tbl
...
#9.Binding Data by Row or by Column with bind_rows() and bind_col()
